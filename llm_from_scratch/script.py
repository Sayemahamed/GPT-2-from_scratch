...
def main():
    ...
    while True:
        ...
        mode = (
            input(
                "Enter 'train' to train, 'generate' to generate text, or 'exit' to quit: "
            )
            .strip()
            .lower()
        )
        if mode == "exit":
            ...
            text_file = input(
                "Enter the path to a text file for training data (or leave blank to enter text manually): "
            ).strip()
            if text_file:
                try:
            ...
            weight_path = input(
                "Enter a filename to save model weights (default: model_weights.pt): "
            ).strip()
            if not weight_path:
            ...
            print("\n--- Generation Mode ---")
            weight_path = input(
                "Enter the path to load model weights (default: model_weights.pt): "
            ).strip()
            if not weight_path:
                weight_path = "model_weights.pt"
            ...
            context_size = (
                model.context_length if hasattr(model, "context_length") else 128
            )
            out = generate_text(
                model, idx, max_new_tokens=max_new_tokens, context_size=context_size
            )
